...
for(g in nodenr:nodenr){
  ...
  # # allocate watersheds on the table template
  # tab <- readRDS(paste0(dir_master,'ssp/tab_template.rds'))
  # # extract and saveback as tab_template_hydro
  # tab@data$ws <- extract(raster(paste0(dir_data,'pcrglobwb_hydrography/basins_5min.tif')),tab)
  # saveRDS(tab,paste0(dir_master,'ssp/tab_template_ws.rds'))

  # calculate current occurrence ranges with dispersal within the watershed
  # calculate future occurrence ranges with dispersal
  
  # cannot calculate dispersal for exclusively lentic species
  filter_lentic_out <- TRUE #override config
  ...
  dir_out <- dir_(paste0(dir_model,clmod,'/modelled_occurrence_dispersal/'))
  ...
  #   sp <- nich[n,'id_no']
  #   
  #   pts <- readRDS(paste0(dir_master,'ssp/ssp_points_dispersal/',sp,'.rds'))
  #   # expand the points based on the catchment
  #   
  #   tab <- cbind(cbind(pts@data,pts@coords)[1:4], #includes info on which cells are
  ...
  esh_calc <- function(sp){ #,sn
    ...
    for(y in 1:nrow(d)){
      ...
      ts <- ts[-which(unname(apply(th[t$occ == 1,],1,all) == FALSE)),]
      th <- th[-which(unname(apply(th[t$occ == 1,],1,all) == FALSE)),]
      
      occn <- sum(t[-which(unname(apply(th,1,all) == FALSE)),'occ'])
      sel <- t[-which(unname(apply(th[t$occ == 1,],1,all) == FALSE)),'occ'] == 1
      # total suitable cells = sum(apply(th,1,all))
      # cells lost within the range = sum(apply(th[t$occ == 1,],1,all) == FALSE)
      # total = lost - gain # positive = overall lost, negative = overall gain
      
      d$ESH_all_hist[y] <- (sum(apply(th[sel,],1,all) == FALSE) - sum(apply(th,1,all)))/occn*100
      
      d$ESH_all[y] <- (sum(apply(ts[sel,],1,all) == FALSE) - sum(apply(ts,1,all)))/occn*100
      d$ESH_Q[y] <- (sum(apply(ts[sel,paste0(c('Qmi','Qzf'),'_',sn)],1,all) == FALSE) - sum(apply(ts[,paste0(c('Qmi','Qzf'),'_',sn)],1,all)))/occn*100
      d$ESH_T[y] <- (sum(ts[sel,paste0(c('Tma'),'_',sn)] == FALSE) - sum(ts[,paste0(c('Tma'),'_',sn)]))/occn*100
  ...
  out <- paste0(dir_model,clmod,'/ESH_tab_dispersal.csv')
