...
# watersheds layer
ws_lyr <- raster('D:/GLOBIO-Aqua/output_5min_arcgis/basins_5min.tif')

# flow acc layer
fa_lyr <- raster('D:/GLOBIO-Aqua/output_5min_arcgis/flowAcc_5min.tif')
...
ws_size <- merge(ws_size,centr2_,by.x = 'ws_id', by.y = 'bsns_5m')
ws_size <- ws_size[order(ws_size$area,decreasing = T),]
row.names(ws_size) <- NULL

number_of_largest_basins = 212 #to consider
...
# remove duplicates
tab_ws <- foreach(i = levels(tab_ws$name),.combine = 'rbind') %do% tab_ws[tab_ws$name == i,][1,]
...
# p
ggsave(paste0(dir_mod,'figs/Figure_2_richest_basins_per_continent.jpg'),p,width = 160,height = 140,units='mm',dpi = 600)
...
p <- ggplot(df,aes(y = value, x = name, fill = variable)) +
  ...
  coord_flip(ylim = c(0,1.0001),expand = F) +
  facet_grid(continent~.,scales = 'free_y',space = 'free_y',switch = 'y') +
  # facet_wrap(continent~.,scales = 'free_y',switch = 'y',ncol = 2) +
  theme_bw() +
  theme(panel.grid = element_blank(),
        panel.border = element_blank(),
        axis.ticks.y = element_blank(),
        ...
        panel.spacing = unit(1.5, "lines"))
p
ggsave(paste0(dir_mod,'figs/Figure_SI_richest_basins_per_continent_200alt.jpg'),p,width = 200,height = 600,units='mm',dpi = 600)
ggsave(paste0(dir_mod,'figs/Figure_SI_richest_basins_per_continent_200t.jpg'),p,width = 100,height = 600,units='mm',dpi = 600)
